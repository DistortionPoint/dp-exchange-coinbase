...
- **`get_trades/2` and `get_trade_history/2` returned the venue's order.** They are now
  oldest first by `:timestamp`, keeping the venue's order for equal timestamps, as
  `Core.Venue` now documents. Break-verified: the new test fails on the previous code.
...
- **A forwarded `nil` option was used as the value instead of meaning "not set".** This
  ...
  **`:limiter`** returned `nil` from `Supervisor.limiter_name/1`: the limiter started
  unnamed while REST carried `limiter: nil` down to `Core.DefaultRateLimiter`, which resolved
  it to its own global default — a different limiter, usually not running, so calls failed
  closed against a venue limiter that was running the whole time.
