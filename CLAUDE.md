...
`dp_exchange_coinbase` is the Coinbase venue package for the **DpExchange** family. It
...
**Never run tier-2 tests on a schedule.** They hit Coinbase's live public API, and a
venue that sees a package polling it on a timer will rate-limit or block.

## Documentation is the source, not the host adapter

Every claim this package makes about Coinbase comes from **Coinbase's own API
documentation**, committed to `docs/reference/coinbase/`. Not a GitHub SDK, not a
...
divergence gets recorded. `docs/reference/coinbase/extraction-pin.md` records exactly
...
docs/reference/coinbase/# Coinbase's own API documentation, committed verbatim
```
